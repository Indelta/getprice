...
function completeFields() {
    ...
    //Бюджет
    var price = $('#screen5 input[type="radio"]:checked').val();
    $('#screen7 .otvetPrice').html(price);

    //Оплата
    var oplata = $('#screen5-2 input[type="radio"]:checked').val();
    $('#screen7 .otvetOplata').html(oplata);
    ...
    function checkType () {
        ...
        $.each(elMass, function (i, val) {
            ...
            if($(val).prop("checked")){
                ...
                var phMass = $('#screen4 .catalogPart >div');
                $.each(phMass, function (j, val2) {
                    if($(val2).hasClass($(val).attr('id'))){
                        $('#screen4 .catalogPart >div').hide();
                        $(val2).show();
                    }
                });
            }
...
$(document).ready(function (){
    $('input[name="price"]').change(function () {
        $('body #screen5 .quest .stolbec ul li').removeClass('selected');
...
//способ оплаты
$(document).ready(function (){
    $('input[name="oplata"]').change(function () {
        $('body #screen5-2 .quest ul li').removeClass('selected');
        $(this).closest('li').addClass('selected');
    });
});
...
$(document).ready(function () {
    completeFields();
    //Экран-агрегатор
    $('#screen5-2 .next, #screen5-2 .openNextStep, #screen5-2 .steps').click(function () {
        completeFields();
...
$(document).ready(function () {
    ...
    $('body #screen2 .plashka .sticker a').click(function () {
        $('#consultationForm').removeClass();
        $('#consultationForm').addClass('rassrochkaClass');
        $('#consultationForm input[name="type"]').attr('value','Кухня в рассрочку');
    });
        ...
        if (phone.val().replace(/\D+/g,"").length<7) {
            ...
            $.ajax({
                ...
                success: function (message) {
                    console.log(message);
                    yaCounter48483560.reachGoal('ZakazZvonkaVsplivashka');
            ...
            $.ajax({
                ...
                success: function (message) {
                    ...
                    if($('#consultationForm').hasClass('consultation')){
                        yaCounter48483560.reachGoal('Consultation');
                        ...
                        yaCounter48483560.reachGoal('Stoimost');
                        gtag('event', 'send', {'event_category': 'Btn', 'event_action': 'Click', 'event_label': 'Stoimost' });
                    } else if($('#consultationForm').hasClass('rassrochkaClass')){
                        yaCounter48483560.reachGoal('Rassrochka');
                        gtag('event', 'send', {'event_category': 'Btn', 'event_action': 'Click', 'event_label': 'Rassrochka' });
                    } else {
                        yaCounter48483560.reachGoal('ZakazZvonka');
    ...
    $('#kalkulatorForm').submit(function () {
        ...
        if (phone.val().replace(/\D+/g,"").length<7) {
            phone
                .animate({backgroundColor: "#ff0000", opacity: 0.8}, 300)
                .animate({backgroundColor: "#fff", opacity: 1}, 300);
            but.prop( 'disabled', false ).val('Получить стоимость кухни со скидкой до 30%');
        }
            ...
            $.ajax({
                ...
                cache: false,
                success: function () {
                    yaCounter48483560.reachGoal('RaschetFinal');
                    gtag('event', 'send', {'event_category': 'Btn', 'event_action': 'Click', 'event_label': 'RaschetFinal' });
                    $('body #screen8 .plashka .blank').fadeOut(300, function () {
                        form.trigger('reset');
                        $(this).after("<div class='thankYou'>Спасибо за Ваш запрос. Менеджер свяжется с Вами в ближайшее время</div>");
                    });
                    setTimeout(function () {
                        $('body #screen8 .plashka .blank').fadeIn(300).siblings('.thankYou').remove();
                        but.prop( 'disabled', false ).val('Получить стоимость кухни со скидкой до 30%');
                        $('body .screen').hide();
                        $('body #screen1').show();
                    }, 10000);
