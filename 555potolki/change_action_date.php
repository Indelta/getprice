<?php
    // если разница между датами из файла action_date.txt и текущей датой составляет меньше недели,
    ...
    function change_action_date() {
        ...
        $week = 60*60*24*7;
        $month = 60*60*24*30;
        if ((strtotime($action_date) - strtotime($now_date)) > $week) {
            echo $action_date;
        }
        else {
            $next_date = strtotime($now_date) + $month;
